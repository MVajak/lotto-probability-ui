...
interface SeasonalPatternsCardProps {
  seasonalPatterns: NonNullable<NumberDetailDto['seasonalPatterns']>;
}
...
export const SeasonalPatternsCard: React.FC<SeasonalPatternsCardProps> = ({ seasonalPatterns }) => {
  ...
  const { byDayOfWeek, byMonth, interpretation } = seasonalPatterns;
  ...
  const dayStats = useMemo(() => {
    const activeDays = byDayOfWeek.filter((d) => d.totalDraws > 0);
    const maxFreq = Math.max(...activeDays.map((d) => d.frequency), 0.01);
    const avgFreq = activeDays.length > 0 ? activeDays.reduce((sum, d) => sum + d.frequency, 0) / activeDays.length : 0;
    return { maxFreq, avgFreq };
  }, [byDayOfWeek]);

  const monthStats = useMemo(() => {
    const activeMonths = byMonth.filter((m) => m.totalDraws > 0);
    const maxFreq = Math.max(...activeMonths.map((m) => m.frequency), 0.01);
    const avgFreq =
      activeMonths.length > 0 ? activeMonths.reduce((sum, m) => sum + m.frequency, 0) / activeMonths.length : 0;
    return { maxFreq, avgFreq };
  }, [byMonth]);
